...
<body>
    ...
    <div class="title">
    <h1>No Pembelian: {{$pembelian->no_pembelian}}</h1>
    <p><b>Nama Vendor: {{$pembelian->vendor->nama_vendor}}</b></p>
  ...
  <hr>
	<table rules="rows" class="grid" style="font-size: 10pt; vertical-align: top; width: 27cm" border="1">
            ...
            <tbody>
            @foreach ($pembeliandetail as $rows)
                  <tr>
                    <td>{{$rows->no_pembelian}}</td>
                    <td>{{$rows->produk->nama_produk}}</td>
                    <td>{{$rows->satuan->nama_satuan}}</td>
                    <td>{{$rows->qty}}</td>
                    <td>{{ number_format($rows->harga,2,",",".")}}</td>
                    <td>{{ number_format($subtotal = $rows->harga * $rows->qty,2,",",".")}}</td>
                  </tr>
            @endforeach
            </tbody>
            <tfoot>
